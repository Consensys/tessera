...
public class ReceiveIT {
    ...
    public void fetchExistingTransactionNotUsingKey() throws UnsupportedEncodingException {
        ...
        final Response response =
                client.target(partyOne.getQ2TUri()).path(RECEIVE_PATH).path(encodedHash).request().buildGet().invoke();
    ...
    public void fetchNonexistantTransactionFails() {
        ...
        final Response response =
                client.target(partyOne.getQ2TUri()).path(RECEIVE_PATH).path("invalidhashvalue").request().get();
