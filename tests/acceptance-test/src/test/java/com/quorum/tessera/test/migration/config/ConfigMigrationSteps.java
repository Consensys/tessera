...
public class ConfigMigrationSteps implements En {
  ...
  public ConfigMigrationSteps() {
    ...
    Given(
        "^(.+) exists$",
        (String filePath) -> assertThat(getClass().getResource(filePath)).isNotNull());
    ...
    Then(
        ...
        (String legacyPath) -> {
          final Config migratedConfig =
              JaxbUtil.unmarshal(Files.newInputStream(outputFile), Config.class);

          // TODO These values were retrieved from legacy.toml.  Ideally legacyConfig would be
          // generated by
          // unmarshalling legacy.toml but didn't want to use the toml unmarshalling production code
          // in the
          // test
          final SslConfig sslConfig = new SslConfig();
          sslConfig.setTls(SslAuthenticationMode.STRICT);
          sslConfig.setServerTlsCertificatePath(
              Paths.get("data", "tls-server-cert.pem").toAbsolutePath());
          ...
          sslConfig.setClientTlsCertificatePath(
              Paths.get("data", "tls-client-cert.pem").toAbsolutePath());
