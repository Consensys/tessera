...
public class NodeExecManager implements ExecManager {
    ...
    public NodeExecManager(ConfigDescriptor configDescriptor) {
        ...
        this.pid = Paths.get(System.getProperty("java.io.tmpdir"), "node-" + configDescriptor.getAlias().name() + ".pid");
    ...
    public Process doStart() throws Exception {
        ...
        if (executionContext.getDbType() != DBType.H2) {
            final Path jdbcJar = Paths.get(System.getProperty("jdbc." + executionContext.getDbType().name().toLowerCase() + ".jar"));
