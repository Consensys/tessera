...
public class DispatchingKeyVaultHandler implements KeyVaultHandler{
    ...
    private Map<KeyVaultType,? extends KeyVaultHandler> lookup = Map.of(
    ...
    public KeyVaultConfig handle(KeyVaultConfigOptions configOptions) {
        KeyVaultType keyVaultType = Optional.ofNullable(configOptions)
            .map(KeyVaultConfigOptions::getVaultType).orElse(KeyVaultType.AWS);
