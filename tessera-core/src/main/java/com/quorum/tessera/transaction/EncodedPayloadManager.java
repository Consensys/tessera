...
/**
 * The EncodedPayloadManager handles requests for translating and validating
 * an incoming request to de/encrypt to pass to the enclave.
 */
public interface EncodedPayloadManager {

    /**
     * Validates an encryption request before passing it to the enclave.
     * This includes deduplicating recipients and checking the affected contracts are valid
     * for the given privacy mode.
     *
     * @param request the full encryption request
     * @return the encrypted data, along with all the information needed for
     *         the sender or any of the recipients to decrypt it
     */
     ...
     * @param maybeDefaultRecipient the key to use to decrypt the payload. If provided, only that key is checked, which may result in decryption failure.
     *                              If {@code null} then locally available keys are tested until a match is found.
