...
public class PushBatchRequestTest {
  ...
  public void create() {
    ...
    PushBatchRequest request = PushBatchRequest.from(payloads, EncodedPayloadCodec.UNSUPPORTED);

    assertThat(request).isNotNull();
    assertThat(request.getEncodedPayloads())
        .containsExactly("payload".getBytes(), "another".getBytes());
    assertThat(request.getEncodedPayloadCodec()).isEqualTo(EncodedPayloadCodec.UNSUPPORTED);
  }
}
