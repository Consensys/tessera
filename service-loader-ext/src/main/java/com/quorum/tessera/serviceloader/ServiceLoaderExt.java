...
public final class ServiceLoaderExt<S> {
  ...
  public Optional<S> findFirst() {
    return serviceLoader.stream().map(ServiceLoader.Provider::get).findFirst();
  }
  ...
  public void forEach(Consumer<? super S> action) {
    stream().map(ServiceLoader.Provider::get).forEach(action);
  }

  public Spliterator<S> spliterator() {
    return stream().map(ServiceLoader.Provider::get).spliterator();
  }

  public Iterator<S> iterator() {
    return stream().map(ServiceLoader.Provider::get).iterator();
  }

  public Stream<ServiceLoader.Provider<S>> stream() {
    return serviceLoader.stream().map(ProviderExt::new);
  }
