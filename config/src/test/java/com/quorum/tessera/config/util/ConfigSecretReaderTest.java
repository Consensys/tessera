...
public class ConfigSecretReaderTest {
    ...
    public void testReadSecret() {
        ...
        when(environmentVariableProvider.getEnv(EnvironmentVariables.CONFIG_SECRET_PATH)).thenReturn(filePath);
        when(environmentVariableProvider.hasEnv(EnvironmentVariables.CONFIG_SECRET_PATH)).thenReturn(true);
    ...
    public void testNotAbleToReadSecret() {
        when(environmentVariableProvider.hasEnv(EnvironmentVariables.CONFIG_SECRET_PATH)).thenReturn(true);
        when(environmentVariableProvider.getEnv(EnvironmentVariables.CONFIG_SECRET_PATH)).thenReturn("not-existed");
    ...
    public void envNotSet() {
        when(environmentVariableProvider.hasEnv(EnvironmentVariables.CONFIG_SECRET_PATH)).thenReturn(false);
        assertThat(configSecretReader.readSecretFromFile()).isEmpty();
        verify(environmentVariableProvider).hasEnv(EnvironmentVariables.CONFIG_SECRET_PATH);
    }
    ...
    public void testReadException() throws IOException {
        ...
        when(environmentVariableProvider.getEnv(EnvironmentVariables.CONFIG_SECRET_PATH)).thenReturn(tempFile.getAbsolutePath());
