...
public class StagingEntityDAOImpl implements StagingEntityDAO {
    ...
    private static final String FIND_HASH_EQUAL
        = "SELECT st FROM StagingTransaction st WHERE st.hash.hash = :hash";
    ...
    private static final String COUNT_STAGED = "SELECT count(st) from StagingTransaction st where st.validationStage is not null";
    ...
    public List<StagingTransaction> retrieveTransactionBatchOrderByStageAndHash(int offset, int maxResults) {
        LOGGER.info("Fetching batch (offset:{},maxResults:{}) of StagingTransaction database rows order by stage and hash", offset, maxResults);
    ...
    public void delete(final MessageHashStr hash) {
        ...
        final StagingTransaction message = entityManager
