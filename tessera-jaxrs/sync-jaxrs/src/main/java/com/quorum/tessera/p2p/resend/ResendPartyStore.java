...
public interface ResendPartyStore {
     ...
     *
     * <p>Sets the number of attempts for each party to 0, even if the party already exists in the list with a higher
     * attempt count.
     *
     * <p>Duplicates are allowed and will be treated individually when making requests. i.e. adding the same url twice
     * will allow twice the limit of maximum attempts
     *
     ...
     *
     * @return Returns {@link Optional#empty()} if there are no parties to contact in the request queue else an Optional
     *     containing a party to contact
     */
    Optional<SyncableParty> getNextParty();

    /**
     * Submit a party that had been attempted to contact but failed This will increase its attempt count or discard it
