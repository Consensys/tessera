...
public enum Launcher {
    NORMAL {
        ...
        public void launchServer(Config config) throws Exception {
            ...
            config.getServerConfigs().forEach(c -> {
                ...
                config.getServerConfigs().stream()
                    ...
                    .map(
                        conf -> {
                            ...
                            ServiceLoader.load(TesseraApp.class).stream().forEach(app -> {
                                LOGGER.debug("Loaded app {}",app.type());
                            });

                            Object app = ServiceLoader.load(TesseraApp.class).stream()
                            ...
                            return TesseraServerFactory.create(conf.getCommunicationType()).createServer(conf, Set.of(app));
                        })
    ...
    RECOVERY {
        ...
        public void launchServer(Config config) throws Exception {
            ...
            final Object app = ServiceLoader.load(TesseraApp.class).stream()
                .map(a -> a.get())
