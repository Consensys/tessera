...
/**
 * An {@link Enclave} provides encryption/decryption functions and keeps hold of all the nodes private keys so the do
 * not leak into other services.
 */
public interface Enclave extends Service {

    /**
     * Retrieves the public key to use if no key is specified for an operation There is no guarantee this key remains
     * the same between runs of the Enclave.
     *
    ...
    /**
     * Returns a set of public keys that should be included as recipients to all transactions produced by this node. The
     * keys are not be managed by this node.
     *
    ...
    /**
     * Encrypts a message using the specified sender and a list of recipients. Returns a {@link EncodedPayload} which
     * contains all the encrypted information, including the recipients and their encrypted master keys.
     *
     ...
     * key that is related to the given public key.
     *
     * @param message the message to be encrypted
