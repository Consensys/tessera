...
/**
 * An {@link Enclave} provides encryption/decryption functions and keeps hold of
 * all the nodes private keys so the do not leak into other services.
 */
public interface Enclave extends Service {

    /**
     * @deprecated This needs moving out of enclave as is basically a local node
     * config property Retrieves the public key to use if no key is specified
     * for an operation There is no guarantee this key remains the same between
     * runs of the Enclave.
     *
    ...
    /**
     * @deprecated This needs moving out of enclave as is basically a local node
     * config property Returns a set of public keys that should be included as
     * recipients to all transactions produced by this node. The keys are not be
    ...
    /**
     * @deprecated This needs moving out of enclave as is basically a local node
     * config property Returns all the public keys that are managed by this
     * Enclave.
     *
     ...
     * @return the encrypted information, represented by an
     * {@link EncodedPayload}
     */
     ...
     *
     * @param rawTransaction the transactiopn to decrypt and re-encrypt with
     * recipients
    ...
    /**
     * Encrypt a payload without any recipients that can be retrieved later. The
     * payload is encrypted using the private key that is related to the given
     * public key.
     *
     * @param message the message to be encrypted
     ...
     * @param providedKey the key to use for decryption, if the payload wasn't
     * sent by this Enclave
     * @return the original, decrypted message
     */
    byte[] unencryptTransaction(EncodedPayload payload, PublicKey providedKey);

    /**
     * Creates a new recipient box for the payload, for which we must be the
     * originator. At least one recipient must already be available to be able
