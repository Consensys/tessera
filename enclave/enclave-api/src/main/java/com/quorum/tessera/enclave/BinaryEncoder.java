...
public interface BinaryEncoder {
    ...
    /**
     * Converts a byte array to its binary form, which takes the following: 8 bytes to describe the number of bytes long
     * the data is the bytes of the data
     *
     ...
     *
     * <p>8 bytes to describe the number of elements and for each element: - 8 bytes to describe the number of bytes
     * long the element is - the bytes of the element
     *
     * @param data the input array to encode
     * @return the encoded byte array
     */
    default byte[] encodeArray(final List<byte[]> data) {
        ...
        return ByteBuffer.allocate(Long.BYTES + elements.length).putLong(data.size()).put(elements).array();
    }
}
