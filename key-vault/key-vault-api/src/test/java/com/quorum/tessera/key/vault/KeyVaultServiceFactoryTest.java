    ...
    public void getInstance() {
        ...
        for(KeyVaultType keyVaultType : KeyVaultType.values()) {
            ...
            when(otherKeyVaultServiceFactory.getType())
                .thenReturn(Stream.of(KeyVaultType.values()).filter(k -> k != keyVaultType).findAny().get());
            ...
            try (var mockedStaticServiceLoader = mockStatic(ServiceLoader.class)) {
                ...
                mockedStaticServiceLoader.when(() -> ServiceLoader.load(KeyVaultServiceFactory.class))
