...
public class RawTransactionMigrator {
    ...
    public void migrate() {
        ...
        final long secondaryTxCount = secondaryEntityManager.createQuery("select count(e) from EncryptedRawTransaction e",Long.class).getSingleResult();
        final int batchCount = calculateBatchCount(maxBatchSize, secondaryTxCount);

        IntStream.range(0, batchCount)
                .map(i -> i * maxBatchSize)
                .mapToObj(offset -> secondaryEntityManager
                ...
                .forEach(
                        ert -> {
                            final Optional<EncryptedRawTransaction> existing = Optional.ofNullable(primaryEntityManager.find(EncryptedRawTransaction.class, ert.getHash()));
                            if (existing.isEmpty()) {
