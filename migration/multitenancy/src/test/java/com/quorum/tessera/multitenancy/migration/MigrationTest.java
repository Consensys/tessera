...
public class MigrationTest {
    ...
    public void beforeTest() throws IOException {
        ...
        primaryEntityManagerFactory = Optional.of(primaryConfig)
        ...
        secondaryEntityManagerFactory = Optional.of(secondaryConfig)
            .map(Config::getJdbcConfig)
            .map(JdbcConfigUtil::toMap)
            .map(m -> new HashMap(m))
            .map(p -> {
    ...
    public void doMigration() {
        ...
        secondaryEntityManager.createQuery("select count(e) from EncryptedTransaction e", Long.class)
            .getResultStream()
            .findFirst()
        ...
        primaryEntityManager.createQuery("select count(e) from EncryptedRawTransaction e", Long.class)
            .getResultStream().findFirst()
            .ifPresent(count -> assertThat(count).isEqualTo(encryptedRawTransactionCount));
    ...
    static EncryptedTransaction generateEncryptedTransaction() {
        ...
        encryptedTransaction.setEncodedPayload(PayloadEncoder.create().encode(generateEncodedPayload()));
        return encryptedTransaction;
    }

    static EncodedPayload generateEncodedPayload() {

        PrivacyMode privacyMode = Arrays.stream(PrivacyMode.values())
            .skip((int) (PrivacyMode.values().length * Math.random()))
            .findAny().get();

        PublicKey senderKey = PublicKey.from("SenderKey".getBytes());

        EncodedPayload.Builder encodedPayloadBuilder = EncodedPayload.Builder.create()
    ...
    static EncryptedRawTransaction generateEncryptedRawTransaction() {
        final EncryptedRawTransaction secondaryRawTx = new EncryptedRawTransaction(
    ...
    static class TestInfo {
        ...
        public String toString() {
            return "TestInfo{" +
                "encryptedTransactionCount=" + encryptedTransactionCount +
                ", encryptedRawTransactionCount=" + encryptedRawTransactionCount +
                '}';
        }
    }
