...
public class EncryptedKeyMatcher {
  ...
  public EncryptedKeyMatcher(
      final List<Box.KeyPair> keyPairs, final EncryptorHelper tesseraEncryptor) {
    ...
    for (int i = 0; i < transaction.encryptedKeys().length; i++) {
      ...
      for (String possibleRecipientPublicKey : privacyGroupAddresses) {
        PublicKey recipientKey =
            PublicKey.from(Base64.getDecoder().decode(possibleRecipientPublicKey));

        final boolean canDecrypt =
            tesseraEncryptor.canDecrypt(transaction, encryptedKey, recipientKey, privateKey);
      ...
      if (recipientKeys.size() != (i + 1)) {
        ...
        throw new RuntimeException(
            "could not find a local recipient key to decrypt the payload with");
      }
    }
  ...
  public Optional<PublicKey> findRecipientKeyWhenNotSenderAndPrivacyGroupNotFound(
      EncryptedPayload transaction) {
    ...
    for (int i = 0; i < transaction.encryptedKeys().length; i++) {
      ...
      for (String ourPublicRecipientKey : ourPublicKeysBase64) {
        ...
        final boolean canDecrypt =
            tesseraEncryptor.canDecrypt(transaction, encryptedKey, senderKey, ourPrivateKey);
        if (canDecrypt) {
