...
public class MigrateConfigCommand implements Callable<Config> {
  ...
  public MigrateConfigCommand(
      Path orionConfigFile, Path outputFile, TesseraJdbcOptions tesseraJdbcOptions) {
  ...
  private Config createConfig() {
    ...
    if (p2pUri.getPort() != p2pPort) {
      p2pUri =
          URI.create(String.format("%s://%s:%s", p2pUri.getScheme(), p2pUri.getHost(), p2pPort));
    }

    final String p2pBindingAddress = toml.getString("nodenetworkinterface", "127.0.0.1");
    final URI p2pBindingUri =
        URI.create(
            String.format("%s://%s:%s", p2pUri.getScheme(), p2pBindingAddress, p2pUri.getPort()));
    ...
    if (q2tPort != qt2Uri.getPort()) {
      qt2Uri =
          URI.create(String.format("%s://%s:%s", qt2Uri.getScheme(), qt2Uri.getHost(), q2tPort));
    }

    final String qt2BindingAddress = toml.getString("clientnetworkinterface", "127.0.0.1");

    final URI qt2BindingUri =
        URI.create(
            String.format("%s://%s:%s", qt2Uri.getScheme(), qt2BindingAddress, qt2Uri.getPort()));
    ...
    String clientConnectionTlsServerTrust =
        toml.getString("clientconnectiontlsservertrust"); // Trust mode
                ...
                p -> {
                  try {
                    return Files.lines(p)
                        .findFirst()
                        .orElse(
                            String.format("[Error: No lines found in file %s", p.toAbsolutePath()));
                  } catch (IOException e) {
