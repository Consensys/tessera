...
public class SslConfigBuilder {
  ...
  public SslConfig build() {
    ...
    if (Stream.of(serverTlsKey, clientTlsKey, tlsServerTrustMode, tlsClientTrustMode)
    ...
    sslConfig.setClientTrustCertificates(
        Stream.ofNullable(clientTrustCertificates)
            .flatMap(Collection::stream)
            .map(Paths::get)
            .collect(Collectors.toList()));

    sslConfig.setServerTrustCertificates(
        Stream.ofNullable(serverTrustCertificates)
            .flatMap(Collection::stream)
            .map(Paths::get)
            .collect(Collectors.toList()));

    Optional.ofNullable(sslConfigType)
        .map(SslConfigType::valueOf)
        .ifPresent(sslConfig::setSslConfigType);
