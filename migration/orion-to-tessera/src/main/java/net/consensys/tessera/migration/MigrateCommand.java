...
public class MigrateCommand implements Callable<Config> {
  ...
  public Config call() throws Exception {
    ...
    MigrateConfigCommand migrateConfigCommand =
        new MigrateConfigCommand(orionKeyHelper.getFilePath(), outputFile, tesseraJdbcOptions);
    Config config = migrateConfigCommand.call();

    try (OutputStream outputStream =
        new TeeOutputStream(Files.newOutputStream(outputFile), System.out)) {
      ...
      Objects.requireNonNull(
          orionConfig.storage(), "Storage config is required. Not found in toml or env");
      ...
      if (migrationInfo.getRowCount() == 0) {
        throw new IllegalStateException(
            String.format(
                "No data found for %s. Check orion storage config string and/or storage env",
                inboundDbHelper.getStorageInfo()));
      }
      ...
      System.out.printf(
          "Migrated %s of %s transactions",
          outcome.get(PayloadType.ENCRYPTED_PAYLOAD), migrationInfo.getTransactionCount());
      System.out.println();
      System.out.printf(
          "Migrated %s of %s privacy groups",
          outcome.get(PayloadType.PRIVACY_GROUP_PAYLOAD), migrationInfo.getPrivacyGroupCount());
      System.out.println();
