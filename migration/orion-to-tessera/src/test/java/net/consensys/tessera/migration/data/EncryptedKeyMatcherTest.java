...
public class EncryptedKeyMatcherTest {
  ...
  public void findRecipientKeyWhenNotSenderAndPrivacyGroupNotFound() {
    ...
    when(encryptor.canDecrypt(
            same(encryptedPayload),
            same(encryptedKey),
            any(PublicKey.class),
            any(PrivateKey.class)))
        .thenReturn(true);

    Optional<PublicKey> result =
        encryptedKeyMatcher.findRecipientKeyWhenNotSenderAndPrivacyGroupNotFound(encryptedPayload);

    assertThat(result).isPresent().contains(PublicKey.from("PublicKeyBytes".getBytes()));
