...
public class RecipientBoxHelperTest {
    ...
    public void resolveRecipientBoxes() {
        ...
        EncryptedKey[] encryptedKeys = Stream.of("ONE","TWO","THREE")
            .map(String::getBytes)
        ...
        List<String> keys = IntStream.range(0,3)
            .mapToObj(i -> {
        ...
        when(privacyGroupPayload.addresses())
            .thenReturn(addresses);

        Box.SecretKey privateKey = mock(Box.SecretKey.class);
        List<Box.KeyPair> pairs = Arrays.stream(addresses)
        ...
        List<String> encodedKeys = results.keySet().stream()
            .map(PublicKey::encodeToBase64)
            .collect(Collectors.toList());

        assertThat(encodedKeys).isEqualTo(keys);

        List<String> values = results.values().stream()
