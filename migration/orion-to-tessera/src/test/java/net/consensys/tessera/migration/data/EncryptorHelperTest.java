...
public class EncryptorHelperTest {
  ...
  public void canDecrypt() {
    ...
    boolean result =
        encryptorHelper.canDecrypt(encryptedPayload, encryptedKey, publicKey, privateKey);
    ...
    verify(encryptor)
        .openAfterPrecomputation(
            cipherText, new Nonce(new byte[24]), MasterKey.from("decryptedKeyData".getBytes()));
  }
}
