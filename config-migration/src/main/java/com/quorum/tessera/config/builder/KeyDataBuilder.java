  ...
  public KeyConfiguration build() {
    if (publicKeys.size() != privateKeys.size()) {
      throw new ConfigException(
          new RuntimeException("Different amount of public and private keys supplied"));
    }
    ...
    return new KeyConfiguration(
        privateKeyPasswordFilePath,
        null,
        keyData.stream().map(KeyDataUtil::marshal).collect(toList()),
        null,
        null);
  }
}
